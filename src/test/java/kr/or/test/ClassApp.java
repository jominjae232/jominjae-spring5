 ...
 *
 */
//오브젝트 생성에 필요한 new 키워드 생성자 메서드 사용하는 실습(아래)
class Circle {
...
class Salesman extends Employee {//extends확장 -> 상속
	public Salesman() {//클래스명과 똑같은 이름의 메서드 생성자 함수입니다.
		//자동으로 생성되는데, 개발자가 커스터마이징 할 때 만듭니다.(아래)
		mDept = "판매 부서";//부모 클래스의 변수를 상속 받아서 사용 가능
	}
	public void doJob() {
...
abstract class GraphicObject {
	...
	//추상메서드를 만드는 이유는 메서드가 수십개, 수백개일 때 개발자 구현 할 때 어려움이 존재.
	//위 문제를 해소한 기능의 클래스 수백개의 메서드를 이름만 모아서, 유지보수가 편리하게 처리한 방식입니다.
}
//추상클래스를 사용하는 방법(아래)
//스프링에서는 추상클래스보다는 인터페이스를 더 많이 사용합니다. 사용 의도 비슷합니다. 
class Treiangle extends GraphicObject {
	//삼각형 그리기
	@Override //부모 클래스의 메서드를 재정의해서 사용 = 오버라이드 / 오버로드 차이점 확인(step1.java확인)
	void draw() {
		// 삼각형을 만듬
		System.out.println("*");
		System.out.println(" * * ");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		// 개발자가 입력한 반지름의 원의 넓이를 구하는 오브젝트를 생성합니다(아래)
