...
public class CommonUtil {
	...
	//다운로드 처리도 같은 페이지에서 결과값만 반환 받는 @ResponseBody 사용
	...
	public FileSystemResource download(@RequestParam("save_file_name")String save_file_name, @RequestParam("real_file_name")String real_file_name, HttpServletResponse response) throws Exception {
		...
		response.setContentType("application/download; utf-8");//아래한글, ppt문서등에서 한글이 깨지는 것을 방지하는 코드추가
		real_file_name = URLEncoder.encode(real_file_name);//ie에서 URL한글일때 에러 발생 방지 코드 추가
