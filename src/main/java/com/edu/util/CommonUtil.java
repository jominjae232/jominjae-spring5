...
public class CommonUtil {
	...
	public String fileUpload(MultipartFile file) throws IOException {
		// TODO UUID클래스로 저장될 고유 식별 파일명을 생성 후, 물리적으로 저장
		...
		//file의 MultipartFile클래스형 객체. 클래스형 자료(멤버변수,메서드...)는 직접 파일 저장을 할 수 없음.
		//그래서, 바이트형으로 파싱(변환)해서 저장해야함. ->bit형(010100001)이진 비트형 자료로 변환 필요.
		//자바자료형 정수: byte(bit로구성)<short<int<long, 실수형(소수점): float<double
		byte[] fileData = file.getBytes();//getByte메서드를 데이터를 bit형으로 파싱해서 저장.
