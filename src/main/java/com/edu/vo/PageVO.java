package com.edu.vo;
/**
 * 이 클래스는 공통(회원관리, 게시물 관리)으로 사용하는 페이징처리+검색기능의 클래스
 * @author 조민재
...
public class PageVO {
	...
	private int queryPerPageNum;//쿼리 전용 변수
	private Integer page;//jsp에서발생 자바전용
	private int perPageNum;//UI 하단에 보여줄 페이징 갯수 계산에 필요
	private int startPage;//위 perPageNum으로 구하는 UI하단 페이지 시작 번호
	private int endPage;//위 perPageNum으로 구하는 UI하단 페이지끝 번호
