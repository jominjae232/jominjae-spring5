...
public class HomeController {
	...
	//네아로 로그인때문에 LoginController 클래스로 분리해서 사용합니다. 그래서 아래는 주석처리
...
//		logger.info("디버그 스프링로고사용: " + jspVar);//System.out 대신 logger 객체를 사용
		...
		pageVO.setQueryPerPageNum(3);//겔러리3개, 공지사항5개
		pageVO.setBoard_type("gallery");
		//첨부파일 save_file_names 배열 변수 값을 지정
		List<BoardVO> latestGallery = boardService.selectBoard(pageVO);
				
		for(BoardVO boardVO:latestGallery) {//리스트형 객체를 1개씩 뽑아서 1개 레코드에 입력 반복
		...
		pageVO.setQueryPerPageNum(5);//공지사항5개, 보드타입 필요(세션으로 처리 않됨)
