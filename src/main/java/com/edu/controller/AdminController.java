...
public class AdminController {
	...
	public String board_update(@RequestParam("file")MultipartFile[] files, BoardVO boardVO, PageVO pageVO) throws Exception {
		//기존 등록된 첨부파일 목록 구하기 List(2차원 배열)객체의 크기는 .size() 구함. 기존 파일이 있을때 사용
		...
		for(MultipartFile file:files) {//files[0]=file, files[1]
			if(file.getOriginalFilename() != "") {//전송된 첨부파일이 있다면 실행
				...
				//아래 for목적: jsp폼에서 기존에 1번 위치에 기존 파일이 있으면, 기존 파일을 지우고, 신규 파일을 덮어쓰는 로직.
				for(AttachVO file_name:delFiles) {//기존 파일을 가져와서 반복하면서 지우기 로직.
					if(idx == sun) {
				...
				//신규 파일 업로드
				save_file_names[idx] = commonUtil.fileUpload(file);//jsp폼에서 전송 파일
				real_file_names[idx] = file.getOriginalFilename();//UI용 이름 임시 저장.
