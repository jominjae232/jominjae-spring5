...
public class AspectAdvice {
	...
	public Object sessionManager(ProceedingJoinPoint pjp) throws Throwable {
		...
		String search_keyword = null;//한글 검색시 IE에서 400에러 발생 때문에 추가
		//조인포인트리스트의 객체의 메서드의 Arguments(매개변수)를 뽑아냄 
		for(Object object:pjp.getArgs()) {
			if(object instanceof PageVO) {//AOP실행 메서드중 매개변수 PageVO pageVO객체 판단
				//결과는 게시판, 멤버 서비스에 PageVO사용하는 서비스만 적용됨. 게시판 생성 관리는 적용X
		...
		if(request != null) {//jsp에서 Get,Post 있을때,
			...
			//검색폼이 있는 jsp에서 발생됨. 결과는 검색폼이 없는 입력/수정/삭제에서는 실행 안 됨.
			...
			if(session.getAttribute("session_search_keyword") != null) {
				...
				if(pageVO != null) {//겟,셋 중에 Set할때 pageVO널이면 에러가 발생하기 때문에 추가한 코드
	...
	//이 메서드는 컨트롤러애서 Exception이 발생했을때 여기서 인터셉터(가로채기)해서
	//에러 메세지를 개발자가 작성한 jsp화면에 뿌려주는 기능을 추가 prevPage, exception 변수2 전송함.
	...
	public ModelAndView errorModelAndView(Exception ex, HttpServletRequest request) {
		...
		request.getSession().setAttribute("session_prevPage", referer);//prevPage세션 변수 만듭니다.
		//---------------------------------------------
